...
class MyTransform extends Transform {
            ...
            input.jarInputs.each { JarInput jarInput ->
                ...
                if (shouldProcessPreDexJar(src.absolutePath)){
                    ...
                    while (enumeration.hasMoreElements()) {
                        ...
                        if (entryName.startsWith("com.lucky.lib.studyapp")) {
                            InputStream inputStream = srcJar.getInputStream(jarEntry)
                            //从jar中取出对应的输入流
                            //todothing
                            byte [] bytes = scanClass(inputStream)
                            inputStream.close()
                        }
                    }
                }
    ...
    /**
     * 判断扫描的类的包名是否是 annotationProcessor自动生成路由代码的包名：com.lucky.lib.studyapp
     * @param classFilePath 扫描的class文件的路径
     */
    static boolean shouldProcessClass(String classFilePath) {
        return classFilePath != null && classFilePath.startsWith("com/lucky/lib/studyapp")
    }
